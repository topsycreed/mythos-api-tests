...
test(
  ...
  async ({ request, debugApiCall }) => {
    const credentials =
      await test.step("Generate unique credentials", async () =>
        createUniqueCredentialsFromEnv());
    ...
    const body = await test.step("Read registration response body", async () =>
      (await response.json()) as RegisterResponseBody);
...
test(
  "POST /login returns a JWT token for a registered user",
  { tag: "@auth" },
  async ({ request, debugApiCall }) => {
    const credentials =
      await test.step("Generate unique credentials", async () =>
    ...
    const registerResponse =
      await test.step("Register the user before login", async () =>
        debugApiCall(
          {
    ...
    const body = await test.step("Read login response body", async () =>
